...
(function($) {
	...
	var lcl = function(obj, settings) {
		...
		var lcl_vars = {
			...
			elem_index: false, // current element manage
			gallery_hook_val: false, // gallery hook value - to discard other ones
			preload_all_used: false, // flag to know when complete preload on document's ready has been triggered
			img_sizes_cache: [], // store image sizes after their preload - manage is images manage
		...
		var setup_elems_obj = function($subj) {
			...
			// if single opening - load just that manage
			if(!o.gallery) {
		...
		/* preload images and eventually trigger showing function - if manage not specified, loads current manage */
		var maybe_preload = function(show_when_ready, el_index) {
			var v = lcl_ai_vars;

			// if forced manage is missing - use current one
		...
		var open_lb = function(inst_obj, $clicked_obj) {
			...
			// if there is a clicked element - set selected manage
			if($co) {
			...
			// array or deeplink initialization - check manage existence
			else {
				if(parseInt(v.elem_index, 10) >= v.elems_count) {
					console.error('LC Lightbox - selected manage does not exist');
					return false;
				}
			}
		...
		var setup_code = function() {
			...
			if(!o.counter || v.elems.length < 2) {
				$('.lcl_counter').remove();
			}
			if(!o.img_zoom) {
				$('.lcl_zoom_icon').remove();
			}
		...
		var show_element = function() {
			...
			// global trigger - before element population | args: element object, element manage
			...
			// trigger right before EVERY element showing | args: element manage, element object
			if(!v.is_arr_instance) {
				var $subj = (v.elems_selector) ? $(v.elems_selector) : lcl_curr_obj;
				$subj.first().trigger('lcl_before_show', [el, v.elem_index]);
			}

			// global trigger - before EVERY element showing | args: element object, element manage
		...
		var populate_lb = function(el) {
			...
			if(elem_has_txt(el) && el.type != 'unknown') {
				...
				if(el.author) {
					$('#lcl_txt').append('<h5 id="lcl_author"> ' + el.author + '</h5>');
				}
				if(el.txt) {
		...
		/* switch element - new_el could be "next", "prev" or element manage */
		var switch_elem = function(new_el, slideshow_switch) {
			var v = lcl_ai_vars;
			var carousel = lcl_ai_opts.carousel;

			if(lcl_is_active || v.elems.length < 2 || $('.lcl_switching_elem').length) {
				return false;
			}

			// find and sanitize new manage
			if(new_el == 'next') {
			...
			if(typeof(lcl_slideshow) != 'undefined') {
				...
				// if isn't a slideshow switch and it is active || if isn't carousel and manage is latest one - stop ss
		...
		var fs_img_manag = function() {
			...
			if(lcl_ai_vars.elems[lcl_ai_vars.elem_index].type != 'image') {
				return false;
			}
		...
		lcl_open = function(obj, index) {
			...
			if(!v) {
				...
				console.error('LC Lightbox - cannot open. Unexisting manage');
				return false;
			} else {
				v.elem_index = index;
