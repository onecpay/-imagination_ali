...
public class JsonUtil {
  ...
  public static List<Object> parseList(String jsonStr) {
    ...
    for (int i = 0; i < array.size(); i++) {
      ...
      String valueTmp = parseObject2Str(value);
      if (isJsonArrayString(valueTmp)) {
        list.add(parseList(valueTmp));
      } else if (isJsonObjectString(valueTmp)) {
        list.add(parseMap(valueTmp));
      } else {
  ...
  public static <T> T convert(Object bean, Class<T> descClass) {
    if (BaseBeanUtil.isEmpty(bean)) {
      return null;
    }
    try {
      Map<String, Object> map = parseMap(parseObject2Str(bean));
      List<String> fieldNames = BaseBeanUtil.getAllFieldNames(descClass);
