...
public abstract class BaseClassUtil {
  ...
  private static final String CLASS_JDK_DYNAMIC_AOP_PROXY =
      "org.springframework.aop.framework.JdkDynamicAopProxy";
  private static final Map<String, Class<?>> CLASSCACHE = new WeakHashMap<String, Class<?>>();
  private static final ClassLoader DEFAULTLOADER = BaseClassUtil.class.getClassLoader();
  ...
  public static Class getSuperClassGenricType(final Class<?> cls, final int index) {
    ...
    if (!(params[index] instanceof Class)) {
      logger.warn(
          cls.getSimpleName() + " not set the actual class on superclass generic parameter");
      return Object.class;
    }
  ...
  public static Class<?> findArrayElementType(Class<?> arrayClass) {
    ...
    if (bracketsIndex != beginIndex) {
      className = className.substring(bracketsIndex + 1);
    } else {
      if (endIndex == -1 && className.indexOf(INTERNAL_ARRAY_PREFIX) == -1) {
        // 形如
    ...
    for (int i = 0; i < packages.length; i++) {
      try {
        return Class.forName(
            prefix + packages[i] + PACKAGE_SEPARATOR + className + suffix, false, loader);
      } catch (ClassNotFoundException ignore) {
      }
    }
    if (dimensions == 0) {
